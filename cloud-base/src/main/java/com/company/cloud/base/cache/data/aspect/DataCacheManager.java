package com.company.cloud.base.cache.data.aspect;

import com.alibaba.fastjson.JSON;
import com.company.cloud.base.cache.RedisCache;
import com.company.cloud.base.cache.data.annotation.DataCacheAble;
import com.company.cloud.base.cache.data.annotation.DataCacheRemove;
import com.company.cloud.base.common.CacheBaseKey;
import com.company.cloud.base.common.util.MD5Util;
import com.company.cloud.base.threads.ThreadCachePool;
...
public class DataCacheManager {
    ...
    public Object caching(ProceedingJoinPoint joinPoint, DataCacheAble dataCacheAble) throws Throwable {
        try{
            ...
            Object cacheObject = redisCache.get(CacheBaseKey.REDIS_CACHED_DATA,dataCacheAble.value()+key);
            ...
            if (cacheObject == null) {
                ...
                if (dataCacheAble.time() > 0) {
                    ...
                    redisCache.put(CacheBaseKey.REDIS_CACHED_DATA,dataCacheAble.value()+":"+key, cacheObject, lifecycle);
                } else {
                    redisCache.put(CacheBaseKey.REDIS_CACHED_DATA,dataCacheAble.value()+":"+key, cacheObject);
                }
            }
            return cacheObject;
        }catch (Exception e){
        ...
        ThreadCachePool.execute(() -> {
            try{
                if(dataCacheRemove.allEntries()){
                    redisCache.removeAll(CacheBaseKey.REDIS_CACHED_DATA,dataCacheRemove.value());
                }else{
                    String key = MD5Util.getMD5(makeKey(joinPoint,dataCacheRemove.key()));
                    redisCache.remove(CacheBaseKey.REDIS_CACHED_DATA,dataCacheRemove.value()+":"+key);
                }
            }catch (Exception e){
                logger.error(e.getMessage());
            }

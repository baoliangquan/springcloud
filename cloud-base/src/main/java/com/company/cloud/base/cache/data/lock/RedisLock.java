package com.company.cloud.base.cache.data.lock;
...
public class RedisLock {
	...
	private boolean redisLock(String lockKey, Integer second) {
		...
		long lockNum = redisCache.incr(CacheBaseKey.REDIS_LOCK, lockKey);
		//如果存储的锁资源大于1，即已经被其他渠道加了锁
		if(lockNum>1){
			return false;
		}else{
			redisCache.expire(CacheBaseKey.REDIS_LOCK, lockKey, second);
			return true;
		}
	}
	...
	private void unRedisLock(String lockKey) {
		redisCache.remove(CacheBaseKey.REDIS_LOCK, lockKey);
		isLock.set(false);
	}
}

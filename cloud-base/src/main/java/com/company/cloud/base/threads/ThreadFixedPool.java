package com.company.cloud.base.threads;
...
public class ThreadFixedPool {
	...
	private static synchronized ExecutorService getPool(ThreadKey poolKey) {
		ExecutorService pool = poolMap.get(poolKey.getKey());
	 ...
	 */
	public static <V> Future<V> call(ThreadKey poolKey, Callable<V> task) {
	 ...
	 * @param task 可运行的任务
	 */
	public static void execute(ThreadKey poolKey, Runnable task) {
	 ...
	 * @return 表示任务等待完成的 Future
	 */
	public static Future<?> submit(ThreadKey poolKey, Runnable task) {
		return getPool(poolKey).submit(task);
	}
	 ...
	 * @param poolKey 线程池key
	 */
	public static void shutdown(ThreadKey poolKey) {
	 ...
	 * @param poolKey 线程池key
	 */
	public static void shutdownNow(ThreadKey poolKey) {
		ExecutorService pool = getPool(poolKey);
		if (pool != null) {
			pool.shutdownNow();
	 ...
	 * 获取线程池信息
	 */
	public static String poolInfo(ThreadKey poolKey) {
		ExecutorService pool = getPool(poolKey);
		if (pool != null) {
